...
function OnMsg.ClassesGenerate()
	Msg("ChoGGi_Library_Loaded","ChoGGi_PinExpander")
end
function OnMsg.ChoGGi_Library_Loaded(mod_id)
	if is_loaded or mod_id and mod_id ~= "ChoGGi_PinExpander" then
		return
	end
	is_loaded = true
	...
	function PinsDlg:InitPinButton(button)
		...
		function button.OnPress(button_obj,gamepad)
			...
			if IsKeyPressed(const.vkControl) or Platform.osx and IsKeyPressed(const.vkLwin) then
				return orig_button_OnPress(button_obj,gamepad)
			end

			local str_dome = Trans(1234--[[Dome--]])
			local str_drones = Trans(71--[[Commanding Drones--]])
			local str_state = Trans(3722--[[State--]])
			local str_Overpopulated = Trans(10460--[[<em>Overpopulated Dome</em>--]])
			local str_NotWorking = Trans(7326--[[Not Working--]])
			local str_Power = Trans(79--[[Power--]])
			local str_Water = Trans(681--[[Water--]])
			local str_Oxygen = Trans(682--[[Oxygen--]])

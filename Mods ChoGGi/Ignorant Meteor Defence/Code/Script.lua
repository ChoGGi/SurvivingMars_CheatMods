...
local function TestRemoveMeteor(meteor)
	-- check for rocks first and shoot down
	...
	if mod_IgnoreNoBuildings then
		...
		local query = {meteor:GetQuery()}
		-- close enough, lua rev 1010999
		if query[3] == "Drone" then
			table.remove(query, 3)
		end
		if query[3] == "Colonist" then
			table.remove(query, 3)
		end
		-- an idle rover could be considered worth more than a meteor
--~ 		if query[4] == "BaseRover" then
--~ 			table.remove(query, 4)
--~ 		end

		--
		local objs = GetRealm(meteor):MapGet(table.unpack(query))
		if #objs < 1 then
			return false
		end
	end

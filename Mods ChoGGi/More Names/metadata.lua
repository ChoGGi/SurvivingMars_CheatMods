return PlaceObj("ModDef", {
	"title", "More Names",
	"id", "ChoGGi_MoreNames",
	"steam_id", "1968507022",
	"pops_any_uuid", "f0196c9b-a2ac-4a8f-a3f5-84c93e805e57",
	"lua_revision", 1007000, -- Picard
	"version", 1,
	"version_major", 0,
	"version_minor", 1,
	"image", "Preview.jpg",
	"author", "ChoGGi",
	"code", {
		"Code/Script.lua",
	},
	"TagCosmetics", true,
	"TagOther", true,
	"has_options", true,
	"description", [[
Adds a bunch of "Unique" names to every nation which are (sometimes loosely) associated with space travel.
They're all the same for each, so you might get von Braun as a Chinese botanist.
If you have more names you think should be added; let me know (lists of male or female names).
[url=https://github.com/ChoGGi/SurvivingMars_CheatMods/blob/master/Mods%20ChoGGi/More%20Names/Code/Script.lua#L23]Current Names[/url]

A new game is required to see new unique names.

Includes mod option to disable the transition to only Martian names.
]],
})

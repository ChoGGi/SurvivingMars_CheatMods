...
function OnMsg.ClassesGenerate()
	...
	do -- MapData
		...
		local csv_columns = {
			{"latitude_degree",ss:format(S[6890--[[Latitude--]]],S[302535920001505--[[°--]]])},
			{"latitude",S[6890--[[Latitude--]]]},
			{"longitude_degree",ss:format(S[6892--[[Longitude--]]],S[302535920001505--[[°--]]])},
	...
	do -- ColonistData
		-- these are checked that we only add once
		local skipped_traits,traits_added_csv
		...
		local function BuildSkippedTraits(t)
			skipped_traits = {}
		...
		function ChoGGi.ComFuncs.ExportColonistDataToCSV()
			...
			if not skipped_traits then
				BuildSkippedTraits(ChoGGi_Tables)
			end
			if not traits_added_csv then
				AddTraitsCSV(ChoGGi_Tables)
			end

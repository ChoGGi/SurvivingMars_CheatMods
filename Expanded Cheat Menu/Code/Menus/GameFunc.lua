...
function ChoGGi.MenuFuncs.DeleteGrassBushesTrees()
	...
	ChoGGi.ComFuncs.QuestionBox(
		T(6779--[[Warning]]) .. "!\n" .. T(302535920001258--[[Removes Grass Bushes Trees for that smooth map feel.]]),
		CallBackFunc,
...
function ChoGGi.MenuFuncs.SetGameSpeed()
	...
	ChoGGi.ComFuncs.OpenInListChoice{
		...
		hint = Translate(302535920000933--[[Current speed: %s]]):format(speeds[const.mediumGameSpeed])
			.. "\n" .. Translate(302535920001134--[[%s = base number %s multipled by custom value amount.]]):format(T(302535920000078--[[Custom Value]]), const.mediumGameSpeed),
		skip_sort = true,
	}
end
...
function ChoGGi.MenuFuncs.SetObjectOpacity()
	...
	local hint_loop = Translate(302535920001109--[[Loops though and makes all %s visible.]])

	local item_list = {
		{text = T(302535920001084--[[Reset]]) .. ": " .. T(3984--[[Anomalies]]), value = "Anomaly", hint = hint_loop:format(T(3984--[[Anomalies]]))},
		{text = T(302535920001084--[[Reset]]) .. ": " .. T(3980--[[Buildings]]), value = "Building", hint = hint_loop:format(T(3980--[[Buildings]]))},
		{text = T(302535920001084--[[Reset]]) .. ": " .. T(302535920000157--[[Cables & Pipes]]), value = "GridElements", hint = hint_loop:format(T(302535920000157--[[Cables & Pipes]]))},
		{text = T(302535920001084--[[Reset]]) .. ": " .. T(547--[[Colonists]]), value = "Colonists", hint = hint_loop:format(T(547--[[Colonists]]))},
		{text = T(302535920001084--[[Reset]]) .. ": " .. T(5438--[[Rovers]]) .. " & " .. T(517--[[Drones]]), value = "Unit", hint = hint_loop:format(T(5438--[[Rovers]]) .. " & " .. T(517--[[Drones]]))},
		{text = T(302535920001084--[[Reset]]) .. ": " .. T(3982--[[Deposits]]), value = "SurfaceDeposit", hint = hint_loop:format(T(3982--[[Deposits]]))},
	}
	local c = #item_list
	if obj then
		c = c + 1
...
--~ 		ResTag_Water = "ResTag_Water",
	...
	local itemlist = {
		...
		{text = Translate(3474--[[Mission Sponsor]]), value = "IMM", hint = Translate(302535920001386--[[Can be changed after in %s>%s>%s.]]):format(T(302535920000002--[[ECM]]), T(1635--[[Mission]]), T(302535920000712--[[Set Sponsor]]))},
		{text = Translate(3478--[[Commander Profile]]), value = "rocketscientist", hint = Translate(302535920001386--[[Can be changed after in %s>%s>%s.]]):format(T(302535920000002--[[ECM]]), T(1635--[[Mission]]), T(302535920000716--[[Set Commander]]))},
		{text = Translate(3486--[[Mystery]]), value = "random", hint = Translate(302535920001386--[[Can be changed after in %s>%s>%s.]]):format(T(27--[[Cheats]]), T(302535920000331--[[Mystery Start]]), "")},
		{text = Translate(3482--[[Colony Logo]]), value = "MarsExpress", hint = Translate(302535920001386--[[Can be changed after in %s>%s>%s.]]):format(T(302535920000002--[[ECM]]), T(1635--[[Mission]]), T(302535920000710--[[Change Logo]]))},
...
function ChoGGi.MenuFuncs.TerrainTextureRemap()
	...
	local hint = Translate(302535920000973--[["Change the value (-1) to an index number from Terrain Textures.
Open %s to see all the textures, the tooltips show the texture index."]]):format(T(302535920000623--[[Terrain Texture Change]]))
	...
	for name, count in pairs(used) do
		...
		item_list[c] = {
			...
			hint = Translate(302535920001313--[[Amount used on map: %s]]):format(count)
